...
| ------------- |:----------------:| 
| Inscription|   - **Inscription validée** : le client peut se connecter au site, son inscription a été validée - **Inscription refusée** :le mail indiqué par le client est déjà utilisé **Possibilité de renseiger information client** : le client peut renseigner ses informations | 
| Connexion | - **Connexion validée** : le client peut accéder au site -- **Connexion refusée** : le mot de passe indiqué est erroné, le client ne peut pas accéder au site --**Ajout de thèmes, disponible**: le client peut créer un nouveau thème --**Création d'article, disponible**: le client peut créer un nouvel article pour un thème choisi               |  
| Consulter thème | --**Affichage des thèmes, disponible** : le client peut consulter les thèmes |   
| Ajouter thème |  --**Ajout de thèmes, disponible**: le client peut créer peut créer un nouveau thème --**Ajout de thèmes, indisponible**: le client ne peut créer de fois un thème avec le même titre    | 
